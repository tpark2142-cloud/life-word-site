self.addEventListener('install', event => {
  event.waitUntil(
    caches.open('lifemoment-static-v129').then(cache => cache.addAll([
      './',
      './index.html',
      './robots.txt',
      './sitemap.xml',
      ...
      './manifest.json',
      './assets/css/app.css?v=20260813f',
      './assets/css/seo-pages.css?v=20260620a',
      './assets/js/app.js?v=20260813f',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys
        .filter(key => key !== 'lifemoment-static-v129')
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        caches.open('lifemoment-static-v129').then(cache => cache.put(event.request, copy));
        return response;
