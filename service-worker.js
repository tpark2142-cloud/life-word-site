self.addEventListener('install', event => {
  event.waitUntil(
    caches.open('lifemoment-static-v147').then(cache => cache.addAll([
      './',
      './index.html',
      './robots.txt',
      './sitemap.xml',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys
        .filter(key => key !== 'lifemoment-static-v147')
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        caches.open('lifemoment-static-v147').then(cache => cache.put(event.request, copy));
        return response;
